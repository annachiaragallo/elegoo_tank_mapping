...
static void ApplicationFunctionSet_MetalCarLinearMotionControl(OwlCarMotionControl direction, uint8_t directionRecord, uint8_t speed, uint8_t Kp, uint8_t UpperLimit)
{
  ...
  if (en != directionRecord || Application_FunctionSet.Car_LeaveTheGround == false)
  {
    en = directionRecord;
    yaw_So = Yaw;
  } //加入比例常数Kp  增大反弹作用 //Add the proportional constant Kp to increase the rebound effect
  //R...
...
ApplicationFunctionSet_OwlCarMotionControl(OwlCarMotionControl direction, uint8_t speed)
{
  ...
  case /* constant-expression */
    ...
    if (/*Application_OwlCarxxx0.Functional_Mode == TraceBased_mode || anna tolto*/ Application_OwlCarxxx0.Functional_Mode == CMD_TrajectoryControl)
    {
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void)
{
  ...
  { /*超声波数据更新：作用于避障*/
    ...
    //anna debug Serial.println(UltrasoundDetectionStatus);
  }
  {
...
$ Elegoo & OwlCar Tank Kit & 2019-09
 ...
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void) //anna angolo
{
  static boolean timestamp = true;
  uint16_t ObsData_mm; //超声波数据
  uint16_t ObsData_cm; //超声波数据
  static int initial_dist;
  static int dist_when_left;
  static int dist_when_right;
  static int dir;
  if (Application_OwlCarxxx0.Functional_Mode == TraceBased_mode)
  {
    ...
    if (function_xxx(UltrasoundData_cm, 0, ObstacleDetection)) //前方cm内有障碍物？(避障数据为超声波检测数据：后台周期性更新)
    {
     ...
     if (function_xxx((millis() - MotorRL_time), 0, 7000))
      {
        AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&ObsData_mm /*out*/);
        ObsData_cm = ObsData_mm / 10;
        initial_dist = ObsData_cm;
        ApplicationFunctionSet_OwlCarMotionControl(Left, 180);
        delay(200);
        ApplicationFunctionSet_OwlCarMotionControl(stop_it, 0);//que
        delay(200);
        AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&ObsData_mm /*out*/);
        ObsData_cm = ObsData_mm / 10;
        dist_when_left = UltrasoundData_cm;
        ApplicationFunctionSet_OwlCarMotionControl(Right, 180);
        delay(400);
        ApplicationFunctionSet_OwlCarMotionControl(stop_it, 0);//que
        delay(200);
        AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&ObsData_mm /*out*/);
        ObsData_cm = ObsData_mm / 10;
        dist_when_right = ObsData_cm;
        dir = 3;

        if((dist_when_left != dist_when_right) && (dist_when_left > initial_dist || dist_when_right  > initial_dist)){
          if(dist_when_left > dist_when_right){
               dir = 1;
              ApplicationFunctionSet_OwlCarMotionControl(Right, 180);
              delay(400);
            }
            else if(dist_when_left < dist_when_right){
            dir = 0;
            ApplicationFunctionSet_OwlCarMotionControl(Left, 180);
            delay(500);
              }
        } else { //can't get a better path neither turning left nor right; rotate more to find better chance
          //ApplicationFunctionSet_OwlCarMotionControl(Left, 180);
          //delay(1000);
          dir = 2;
          }
    ...
    else
    {
      timestamp = true;
      /*控制左右电机转动：实现匀速直行*/
      ApplicationFunctionSet_OwlCarMotionControl(Forward, 120);
    }
  }
  else
  {
...
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::ApplicationFunctionSet_Obstacle(void) //anna ostacolo
{
  static boolean timestamp = true;
  static long obs_time = millis();
  if (Application_OwlCarxxx0.Functional_Mode == ObstacleAvoidance_mode)
  {
    ...
    { 
      ...
      if (timestamp == true || millis() - MotorRL_time > 7000)
      {
        /*doc["D"] = UltrasoundData_cm;
        doc["T"] = millis() - obs_time;
        doc["A"] = UltrasoundData_cm;*/
        doc["M"] = 0;
        /*doc["I"] = UltrasoundData_cm;*/
        doc["T"] = (millis() - obs_time) / 1000;
        serializeJson(doc, Serial);
        delay(200);
        timestamp = false;
        MotorRL_time = millis();
...
void ApplicationFunctionSet::ApplicationFunctionSet_Standby(void)
{
  ...
  if (Application_OwlCarxxx0.Functional_Mode == Standby_mode)
  {
    ApplicationFunctionSet_OwlCarMotionControl(stop_it, 0);
         ...
         * StaticJsonDocument<50> doc;
        doc["M"] = "stdby";
        doc["S"] = UltrasoundDetectionStatus;
        doc["D"] = UltrasoundData_cm;
        serializeJson(doc, Serial);*/
...
/*
  数学公式：获取任意坐标值的相对角度 //Mathematical formula: Get the relative angle of any coordinate value
*/
...
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_TrajectoryControl_xxx0(void) //anna calcolo
{
  ...
  if (Application_OwlCarxxx0.Functional_Mode == CMD_TrajectoryControl)
  {
    ...
    //0# 获取运动坐标值的相对角度 //Get the relative angle of the motion coordinate value
    getRngle(New_Record_X - Trajectory_Record_X /*in*/, New_Record_Y - Trajectory_Record_Y /*in*/, &Position_angle_E0 /*out*/);
    //1 #计算平面位置角度偏移误差值 ////1 #Calculate the plane position angle offset error value
    Deviation_angle = Position_angle_E0 - Position_angle_E1; //偏差角<（P-0） - （p-1）:本次 减 上次> //Deviation angle <(P-0) - (p-1): this time minus last time>
    //方向以 Position_angle_E1 为中心点平分左右运动控制范围 ±180° //The direction uses Position_angle_E1 as the center point to bisect the left and right motion control range ±180°
